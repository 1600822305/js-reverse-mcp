...
- **🆕 代码覆盖率**: JavaScript 代码覆盖率分析，识别未使用代码
- **🆕 反混淆**: 变量名还原、控制流分析、字符串解密
...
| `monitor_websocket`          | 监控 WebSocket 连接和消息       |
| `stop_websocket_monitor`     | 停止 WebSocket 监控             |
| `list_websocket_connections` | 列出所有追踪的 WebSocket 连接   |
| `intercept_requests`         | 拦截并修改网络请求              |
| `stop_interceptor`           | 停止请求拦截器                  |
| `list_interceptors`          | 列出所有活动的拦截器            |
| `mock_api_response`          | 快速设置 API 模拟响应           |

### 全局变量监控

| 工具               | 描述                                          |
| ------------------ | --------------------------------------------- |
| `list_globals`     | 列出页面上所有自定义全局变量                  |
| `watch_global`     | 监控全局变量变化，记录修改时的调用栈          |
| `unwatch_global`   | 停止监控全局变量                              |
| `list_watchers`    | 列出所有活动的变量监控器                      |
| `snapshot_globals` | 创建全局变量快照，用于比较                    |
| `diff_globals`     | 比较当前状态与快照的差异，找出新增/修改的变量 |

### 加密检测

| 工具                     | 描述                               |
| ------------------------ | ---------------------------------- |
| `detect_encryption`      | 检测页面中使用的加密库和算法       |
| `find_crypto_functions`  | 在脚本中搜索加密相关函数定义       |
| `analyze_encoded_string` | 分析字符串的编码类型并尝试解码     |
| `hook_crypto_functions`  | 自动 Hook 常见加密库函数，记录调用 |
...
| `stop_js_coverage`     | 停止收集并生成覆盖率报告，识别未使用的代码   |
...
| `restore_control_flow`   | 分析并还原混淆的控制流结构，检测控制流扁平化         |
| `decrypt_strings`        | 解密和解码混淆字符串，支持 Base64、Hex、Unicode 等   |
...
| `monitor_input_changes`       | 监控输入框变化（包括密码框）         |
...
# 3. 生成报告，找出未使用的代码
stop_js_coverage({ minCoverage: 50 })  # 只显示覆盖率低于50%的脚本
```
...
```
# 1. 变量名还原
restore_variable_names({ scriptId: "123", aggressive: true })

# 2. 控制流分析
restore_control_flow({ scriptId: "123", simplify: true })

# 3. 字符串解密
decrypt_strings({ scriptId: "123", autoDetect: true })
```
