...
/**
 * Get all form data on the page.
 */
export const getFormData = defineTool({
  name: 'get_form_data',
  description:
    'Gets all form data from the page including hidden fields. Useful for understanding what data is being submitted.',
  annotations: {
    title: 'Get Form Data',
    category: ToolCategory.REVERSE_ENGINEERING,
    readOnlyHint: true,
  ...
  schema: {
    selector: zod
      ...
      .optional()
      .default('form')
      .describe('CSS selector for the form(s) to inspect.'),
  },
  handler: async (request, response, context) => {
    const {selector} = request.params;
...
(function() {
  ...
  if (forms.length === 0) {
    return { error: 'No forms found matching: ' + selector };
  }
  ...
  forms.forEach((form, index) => {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      if (el.type === 'hidden') {
        formInfo.hiddenFields.push(fieldInfo);
      } else {
...
`;
    ...
    try {
      const page = context.getSelectedPage();
      const result = (await page.evaluate(getDataCode)) as {
        error?: string;
        forms: Array<{
          index: number;
          id: string | null;
      ...
      response.appendResponseLine(
        `📋 Form Data (${result.forms.length} form(s)):\n`,
      );

      for (const form of result.forms) {
        response.appendResponseLine(
          `📝 Form #${form.index}${form.id ? ` (id="${form.id}")` : ''}${form.name ? ` (name="${form.name}")` : ''}`,
        );
        response.appendResponseLine(`   Action: ${form.action}`);
        response.appendResponseLine(`   Method: ${form.method}`);
        response.appendResponseLine(`   Encoding: ${form.encoding}`);
        response.appendResponseLine('');
        ...
        if (form.hiddenFields.length > 0) {
          response.appendResponseLine(
            `   🔒 Hidden Fields (${form.hiddenFields.length}):`,
          );
          for (const field of form.hiddenFields) {
            const valPreview =
              field.value.length > 50
                ...
                : field.value;
            response.appendResponseLine(`     - ${field.name}: ${valPreview}`);
          }
          response.appendResponseLine('');
        }
      }
      ...
      response.appendResponseLine(
        '  - Use monitor_form_submit to capture form submissions',
      );
      response.appendResponseLine(
        '  - Use monitor_input_changes to track password field modifications',
      );
      response.appendResponseLine(
        '  - Hidden fields often contain tokens, signatures, or encrypted data',
      );
    } catch (error) {
      response.appendResponseLine(
        `Error: ${error instanceof Error ? error.message : String(error)}`,
